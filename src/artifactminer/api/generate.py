...
from artifactminer.api.schemas import GenerateArtifactRequest, GeneratedArtifactResponse
from artifactminer.api.views import get_prefs
from artifactminer.db import UploadedZip, get_db
from artifactminer.generators import generate_portfolio_html
...
@router.post("/portfolio", response_model=GeneratedArtifactResponse)
async def generate_portfolio_artifact(
    request: GenerateArtifactRequest,
    db: Session = Depends(get_db),
) -> GeneratedArtifactResponse:
    context = _resolve_portfolio_context(db, request.portfolio_id)
    path = generate_portfolio_html(db, context["portfolio_id"])
    return GeneratedArtifactResponse(
        success=True,
        artifact="portfolio",
        path=str(path.resolve()),
        generated_at=datetime.now(UTC).replace(tzinfo=None),
        warnings=list(getattr(generate_portfolio_html, "last_warnings", [])),
    )
