...
export function FileUpload({ onSubmit, onBack, scanRoot }: FileUploadProps) {
	...
	const doScan = useCallback(async () => {
		...
		if (result.error) {
			...
			toast.show({
				variant: "success",
				message: `Found ${result.zips.length} ZIP files`,
				duration: 3000,
			});
		}
	}, [rootPath]);
	...
	const handleItemClick = (colIdx: number, entryIdx: number) => {
		...
		const currentColumn = newColumns[realColIdx];
			...
			>
				...
				{scanStatus !== "scanning" ? (
					...
					>
						<box width={3} onMouseDown={() => setIsSearchFocused(true)}>
							<text>
								<span fg={isSearchFocused ? theme.gold : theme.textDim}>
									{"🔍"}
								</span>
							</text>
						</box>
				...
				{!searchQuery && scanStatus !== "scanning" ? (
					<box flexDirection="row" alignItems="center" paddingLeft={2}>
						{columns.map((col, i) => {
							const name =
								col.path === rootPath ? "~" : col.path.split("/").pop() || "/";
							...
							return (
								...
								>
									<text>
										<span fg={isLast ? theme.gold : theme.textDim}>
											{isLast ? <strong>{name}</strong> : name}
										</span>
										{!isLast ? <span fg={theme.textDim}>{" / "}</span> : null}
									</text>
								</box>
							);
						})}
				...
				<box flexGrow={1} flexDirection="column" padding={1}>
					<scrollbox flexGrow={1}>
						{searchResults.length === 0 ? (
							<box padding={2} flexDirection="row" justifyContent="center">
								<text>
							...
							searchResults.map((zip, i) => {
								...
								const bg = isSelected
									...
									: i % 2 === 0
										...
										: "#111111";
								const sizeStr =
									zip.size !== undefined ? formatSize(zip.size) : "";
								return (
									<box
										key={zip.fullPath}
										backgroundColor={bg}
										...
										paddingTop={1}
										paddingBottom={1}
										onMouseDown={() => onSubmit(zip.fullPath)}
									>
										<text>
											<span fg={isSelected ? theme.gold : theme.textDim}>
												{"📦 "}
											</span>
											<span fg={isSelected ? theme.gold : theme.textSecondary}>
					...
					{visibleColumns.map((col, colIdx) => {
						...
						return (
							<box
								...
								border
								borderStyle="single"
								borderColor={isActive ? theme.gold : theme.bgLight}
							>
								{/* Column header */}
								<box
									paddingLeft={1}
									paddingRight={1}
									backgroundColor={isActive ? theme.bgMedium : theme.bgDark}
								>
									<text>
										<span fg={isActive ? theme.gold : theme.textDim}>
											<strong>{col.path.split("/").pop() || "~"}</strong>
										</span>
									</text>
								</box>
								...
								<scrollbox flexGrow={1} focused={isActive}>
									{entries.map((entry, i) => {
										...
										const isSelected = i === col.selectedIndex;
										const bg = isSelected
											? isActive
												? "#1a1a00"
												: theme.bgMedium
										...
										return (
											<box
												...
												paddingTop={1}
												paddingBottom={1}
												onMouseDown={() => handleItemClick(colIdx, i)}
											>
												<text>
													<span
														fg={
													...
													{display.detail ? (
														<span fg={theme.textDim}> {display.detail}</span>
													) : null}
													{display.isDir && !("isParent" in entry) ? (
														<span fg={theme.textDim}>{" ›"}</span>
													) : null}
												</text>
											</box>
